...
job_list = ['verbose', 'recon', 'tracula', 'cvs_subj2mni', 'cvs_mni2subj', 'save_contact_coordinates', 'morphcontacts',
...
def run_data_consistency_checks(subj):
...
def setup_jobs(args, allowed_jobs, jobs):
    for k in jobs.keys():
        if 'no' + k in args:
            jobs[k] = False
        else:
...
def run_recon(subj, data_dir, jobs, email_body):
    tstart = time.time()
    ...
    print(log)
    email_body.append(log)
    freesurfer.recon(subj=subj, t1_file=data_dir['t1file'], t2_file=data_dir['t2file'], openmp=openmp,
...
def run_save_contact_coordinates(subj, data_dir, email_body):
    tstart = time.time()
...
def run_cvs_subj2mni(subj, jobs, email_body):
    tstart = time.time()
    ...
    print(log)
    email_body.append(log)
    freesurfer.cvs_subj2mni(subj=subj, openmp=openmp, verbose=jobs['verbose'])
    log = '    + Finished in ' + str((time.time() - tstart) / 3600) + ' hours.'
    print(log)
    email_body.append(log)
    return email_body
...
def run_cvs_mni2subj(subj, jobs, email_body):
    tstart = time.time()
    ...
    print(log)
    email_body.append(log)
    freesurfer.cvs_mni2subj(subj=subj, openmp=openmp, subjects_dir=SUBJECTS_DIR, verbose=jobs['verbose'])
    log = '    + Finished in ' + str((time.time() - tstart) / 3600) + ' hours.'
    print(log)
    email_body.append(log)
    return email_body
...
def run_morphcontacts(subj, email_body):
    ...
    contacts_dir_native = SUBJECTS_DIR_NATIVE + subj + os.sep + 'contacts' + os.sep
    contacts_dir = SUBJECTS_DIR + subj + '/contacts/'
    contacts_cvs_dir_native = SUBJECTS_DIR_NATIVE + subj + os.sep + 'contacts_cvs_avg35' + os.sep
    contacts_cvs_dir = SUBJECTS_DIR + subj + '/contacts_cvs_avg35/'
    ...
    for i in np.arange(0, all_coords.shape[0]):
        ...
        volumes.contact_to_volume(
            ...
            reference_volume=os.path.join(SUBJECTS_DIR_NATIVE, subj, 'mri', 'norm.mgz'),
    ...
    while need_to_rerun:
        contacts_files_in_cvs = [f for f in glob.glob(
            SUBJECTS_DIR_NATIVE + subj + os.sep + 'contacts_cvs_avg35' + os.sep + '*.mgz')]
        ...
        if len(contacts_files) > 0:
            run_cvs_apply_morph(contacts_files, contacts_cvs_dir)
        else:
            need_to_rerun = False

            # update contact coordinates file with mni coordinates and Yeo atlas
            all_coords = pd.read_excel(os.path.join(SUBJECTS_DIR_NATIVE, subj, 'Contact_coordinates.xlsx'))
    ...
    all_coords = volumes.identify_voxel_location(all_coords,
                                                 SUBJECTS_DIR_NATIVE + 'Yeo_JNeurophysiol11_MNI152' + os.sep + 'Yeo2011_7Networks_MNI152_FreeSurferConformed1mm_LiberalMask.nii.gz',
...
def run_convert_dti_dicoms(subj, data_dir, email_body):
    ...
    print(log)
    email_body.append(log)
    data_dir['dwidir'] = RAW_DATA + '//' + subj + '//DWI'
...
def run_tracula(subj, data_dir, email_body):
...
def run_probtrack(subj, jobs, email_body):
...
def run_morphprobtrack(subj, email_body):
...
if __name__ == '__main__':
    ...
    data_dir, allowed_jobs = run_data_consistency_checks(subj)
    jobs = setup_jobs(sys.argv, allowed_jobs, jobs)
    ...
    # if 'morphcontacts' in sys.argv:
    #     morphcontacts = True
    # if 'nomorphcontacts' in sys.argv:
    #     morphcontacts = False
    ...
    # if morphcontacts:
    #     if (os.path.isdir(SUBJECTS_DIR_NATIVE + subj + os.sep + 'cvs' + os.sep) == False) | (
    #         (not cvs_subj2mni) & (not save_contact_coordinates)):
    #         print("WARNING: CVS to MNI Space not found. Use 'cvs_subj2mni' and 'save_contact_coordinates' arguments.")
    #         print('EXECUTION STOPPED.')
    #         sys.exit()
    ...
    if jobs['recon']:
        ...
        # log = '    + Finished in ' + str((time.time() - tstart) / 3600) + ' hours.'
        # print(log)
        email_body = run_recon(subj, data_dir, jobs, email_body)
    ...
    if jobs['save_contact_coordinates']:
        ...
        # print(log)
        # email_body.append(log)
        email_body = run_save_contact_coordinates(subj, data_dir, email_body)
    ...
    if jobs['cvs_subj2mni']:
        ...
        # print(log)
        # email_body.append(log)
        email_body = run_cvs_subj2mni(subj, jobs, email_body)
    ...
    if jobs['cvs_mni2subj']:
        ...
        # print(log)
        # email_body.append(log)
        email_body = run_cvs_mni2subj(subj, jobs, email_body)
    ...
    if jobs['morphcontacts']:
        ...
        # print(log)
        # email_body.append(log)
        email_body = run_morphcontacts(subj, email_body)
    ...
    if (data_dir['dtifile'] is not None) and jobs['tracula']:
        ...
        # print(log)
        # email_body.append(log)
        email_body = run_convert_dti_dicoms(subj, data_dir, email_body)
    else:
    ...
    if jobs['tracula']:
        ...
        # email_body.append(log)
        # paths.silentremove(cfg)
        email_body = run_tracula(subj, data_dir, email_body)
    ...
    if jobs['probtrack']:
        ...
        # print(log)
        # email_body.append(log)
        email_body = run_probtrack(subj, jobs, email_body)
    ...
    if jobs['morphprobtrack']:
        ...
        # print(log)
        # email_body.append(log)
        email_body = run_morphprobtrack(subj, email_body)
    ...
    with open(SUBJECTS_DIR_NATIVE + subj + os.sep + 'log.txt', 'wt') as file:
        file.write('\n'.join(email_body))
